...
<input type="checkbox" id="check2" value="2" onClick="javascript:functionSelect('2')"><span id="text2"> Select all </span>
	...
	<?php
		...
		if ($link = ocilogon("ATLAS_TILECOM_R", "TileBrasil1", "INTR")) {
		} 
		else {
			$err = OCIError();
			echo "Oracle Connect Error " . $err[text];
	    	}
		$queryRunType = "select name from ATLAS_TILECOM.tcaRunType";

		$resultRunType = ociparse($link, $queryRunType);

		ociexecute($resultRunType, OCI_DEFAULT);

		while (ocifetch($resultRunType))
		{
			$resultName = ociresult($resultRunType, "NAME");
			if ($resultName == "")
				$resultName = ociresult($resultRunType, "name");
		    	echo"<option value='". $resultName ."'>". $resultName ."</option>\n";
		}
		
		OCIFreeStatement($resultRunType);
	...
	$queryRunType= 'select id,value  from ATLAS_TILECOM.tcaStatCommentsValues';
	...
	while (ocifetch($resultRunType))
	{
		    if(ociresult($resultRunType, "ID")!= 0)
              	    {
                	echo"<option value='". ociresult($resultRunType, "ID") ."'>";
		    	$value = strtr (rtrim( ociresult($resultRunType, "VALUE") ),"_"," ");
		    	echo"$value</option>\n";
		    }
	}
	OCIFreeStatement($resultRunType);
