...
<body>
	<div id="top-bar">
	<form action="wicomViewStatusCommentTimeline.php" name="selectModule" method="get">
	 Select the RunType: 
					...
					{	
						...
						if($moduleNames[$moduleIndex]== $tempModuleName )
						{
							...
							if( ($detailedComment[$j]==NULL && $detailedComment[$j]=="") || $statusComment[$j]==4)
							{
								...
								echo"<a href=\"\" alt=\"$tempModuleName x $runNumber[$j]  -> No detailed comment\" title=\"$tempModuleName x $runNumber[$j] -> No detailed comment\">&nbsp;</a></TD>";
							}
							else
							{
								echo"\n<TD bgcolor=\"".$color[$statusComment[$j]]."\">";
								echo"<a href=\"javascript:viewDetailed('";
								echo $tempModuleName."',". $runNumber[$j].");\" alt=\"$tempModuleName x $runNumber[$j] -> No detailed comment\" title=\"$tempModuleName x $runNumber[$j] ->  There is detailed comments\"alt=\"$tempModuleName x $runNumber[$j] -> No detailed comment\" title=\"$tempModuleName x $runNumber[$j] -> There is detailed comments\"><img width='15' height='15' src=\"./images/editIcon.png\" border='0'> </a></TD>";
							}
							$found=true;
						}
